...
class PreReceiveGuardTest < MiniTest::Spec
  describe "In general" do
    ...
    it "knows if the hook is called via SSH and allow pushes if local" do
    ...
    it "never denies force-pushes for merge-requests" do
    ...
    it "denies fast forwards if the correct env var is set" do
      @env["GITORIOUS_DENY_FORCE_PUSHES"] = "true"
      guard = Gitorious::SSH::PreReceiveGuard.new(@env,
    ...
    it "builds the correct authentication URL" do
    ...
    it "extracts the Git target correctly" do
      assert_equal "refs/merge-requests/123", @guard.git_target
    end

    it "chomps newlines from the git_target" do
      guard = Gitorious::SSH::PreReceiveGuard.new(@env,
        "#{'0'*10} #{'fca'*10} refs/heads/master\n")
      assert_equal "refs/heads/master", guard.git_target
    end

    it "does not allow push when Gitorious says no" do
      @guard.stubs(:get_via_http).returns("false")
      assert !@guard.allow_push?
    end

    it "allows push when Gitorious says it is ok" do
      @guard.stubs(:get_via_http).returns("true")
      assert !@guard.deny_force_pushes?
      assert @guard.allow_push?
    end

    it "knows if something is a null sha1" do
      assert !@guard.null_sha?("abcd"*10)
      assert @guard.null_sha?("0000000000000000000000000000000000000000")
    end

    it "allows deletion of merge requests on local connections" do
      @env.delete("SSH_ORIGINAL_COMMAND")
      guard = Gitorious::SSH::PreReceiveGuard.new(@env,
    ...
    it "denies deletion of merge requests on remote connections" do
      guard = Gitorious::SSH::PreReceiveGuard.new(@env,
        "#{'0'*10} #{'fca'*10} refs/merge-requests/123\n")
      assert guard.deny_merge_request_update_with_sha?("0"*40)
    end
