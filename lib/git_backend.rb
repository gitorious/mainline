...
class GitBackend
  ...
  class << self
    ...
    def is_git_daemon_export_ok?(repo_path)
      File.exist?(File.join(repo_path, "git-daemon-export-ok"))
    end

    def do_repo_path_exists?(repo_path)
      repo_path.index(BASE_PATH) == 0 || File.directory?(repo_path)
    end
